    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center p-8 bg-slate-50">
            <div className="flex flex-col items-center gap-6">
                ...
                <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-[10px]">Loading Framework...</p>
            </div>
        </div>
    );
    ...
    if (completed) return (
        <div className="min-h-screen flex items-center justify-center p-8 bg-white selection:bg-indigo-100">
            <div className="max-w-md w-full text-center space-y-8 animate-in zoom-in duration-700">
                <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-xl border border-emerald-100 mb-10">
                ...
                <div className="pt-8 border-t border-slate-50 space-y-6">
                    <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 text-left space-y-4 animate-in slide-in-from-bottom-4 duration-1000 delay-300">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100">
                                <span className="material-symbols-rounded text-xl">neurology</span>
                            </div>
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Neural Coaching Lab</h3>
                        </div>
                        ...
                        <button 
                            ...
                            className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center justify-center gap-2 shadow-xl shadow-indigo-100"
                        >
                    ...
                    <button 
                        ...
                        className="text-slate-400 hover:text-slate-900 transition-colors font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-2 mx-auto"
                    >
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 py-12 px-6 lg:py-24">
            <div className="max-w-3xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-1000">
                <header className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/20 space-y-4">
                    <div className="flex justify-between items-start">
                        <span className="bg-indigo-600 text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">
                            {invite.instance.template.survey_type.name}
                        </span>
                        <div className="text-right">
                            <p className="text-slate-300 text-[10px] font-black uppercase tracking-widest">Confidential Entry</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {questions.map((q: any, i: number) => (
                        <div key={q.id} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/10 space-y-8 group transition-all duration-500 hover:border-indigo-600">
                            <div className="flex gap-6 items-start">
                                <span className="w-10 h-10 rounded-2xl bg-slate-50 text-slate-400 font-black text-xs flex items-center justify-center shrink-0 border border-slate-100 shadow-sm group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                    {i + 1}
                                </span>
                            ...
                            {q.type === 'RATING' && (
                                <div className="space-y-6">
                                    <div className="flex justify-between items-end px-2">
                                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Least Agree</span>
                                        <div className="text-4xl font-black text-indigo-600">{responses[i]?.answer_value}</div>
                                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Fully Agree</span>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {JSON.parse(q.options || '[]').map((opt: string, optIdx: number) => (
                                        <button 
                                            ...
                                            className={`w-full p-5 rounded-2xl text-left font-bold text-sm transition-all flex items-center justify-between border-2 ${responses[i]?.answer_value === optIdx ? 'border-indigo-600 bg-indigo-50/50 text-indigo-700' : 'border-slate-50 bg-slate-50/30 text-slate-500 hover:border-slate-200 shadow-sm'}`}
                                        >
                                            {opt}
                            ...
                            {q.type === 'TEXT' && (
                                <textarea 
                                    className="w-full px-8 py-6 bg-slate-50/50 border-none rounded-[2rem] focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-bold text-slate-700 placeholder:text-slate-300 min-h-[140px] transition-all"
                    ...
                    <div className="pt-10 flex flex-col items-center gap-6">
                        <button 
                            type="submit"
                            disabled={submitting}
                            className="px-24 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.3em] shadow-2xl shadow-indigo-200 hover:bg-indigo-600 hover:scale-[1.02] transition-all disabled:opacity-50 active:scale-[0.98]"
                        >
                            {submitting ? 'Transmitting Entry...' : 'Complete Entry'}
                        </button>
                        <p className="text-slate-300 font-black uppercase tracking-widest text-[9px] flex items-center gap-2">
