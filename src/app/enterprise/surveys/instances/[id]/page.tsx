...
export default function SurveyReport({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
            ...
            <section className="relative overflow-hidden bg-slate-900 rounded-[3rem] p-10 md:p-14 text-white shadow-2xl shadow-indigo-200/20 group">
                {/* Decorative Elements */}
                ...
                {!aiAnalysis && !analyzing ? (
                    <div className="flex flex-col md:flex-row items-center justify-between gap-12 relative z-10">
                        ...
                        <button 
                            ...
                            className="px-10 py-5 bg-indigo-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] flex items-center gap-3 hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-900/40 shrink-0"
                        >
                    ...
                    <div className="flex flex-col items-center justify-center py-10 space-y-8 animate-pulse relative z-10">
                        ...
                        <div className="text-center space-y-2">
                            <h3 className="text-xl font-black tracking-widest uppercase">Analyzing Pulse...</h3>
                    ...
                    <div className="space-y-12 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                        <header className="flex flex-col md:flex-row items-start justify-between gap-8 pb-10 border-b border-white/10">
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    <span className="bg-indigo-600 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">Strategic Insight</span>
                                    <span className="text-slate-500 font-black text-[10px] uppercase tracking-widest">Generated by Croar AI</span>
                                </div>
                                <h2 className="text-4xl font-black tracking-tighter leading-none">{report.instance_name} Summary</h2>
                                <p className="text-slate-300 text-lg font-medium max-w-3xl leading-relaxed italic">"{aiAnalysis.summary}"</p>
                            </div>
                            <div className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] text-center min-w-[220px] backdrop-blur-xl shrink-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] px-2">Cultural Strengths</h4>
                                <ul className="space-y-3">
                                    {aiAnalysis.strengths.map((s: string, idx: number) => (
                                        <li key={idx} className="flex items-center gap-4 bg-white/5 p-5 rounded-3xl border border-white/5 group hover:border-emerald-500/50 transition-all">
                            ...
                            <div className="space-y-6">
                                <h4 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em] px-2">Detected Risks</h4>
                                <ul className="space-y-3">
                                    {aiAnalysis.weaknesses.map((w: string, idx: number) => (
                                        <li key={idx} className="flex items-center gap-4 bg-white/5 p-5 rounded-3xl border border-white/5 group hover:border-rose-500/50 transition-all">
                        ...
                        <div className="pt-10 border-t border-white/10 space-y-6">
                            <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] px-2">AI Strategic Recommendations</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {aiAnalysis.recommendations.map((r: string, idx: number) => (
                                    <div key={idx} className="bg-indigo-600/10 border border-indigo-500/20 p-8 rounded-[2.5rem] flex gap-5 group items-start hover:bg-indigo-600 transition-all duration-500">
                                        <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover:bg-white group-hover:text-indigo-600 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/30">
                    <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-1">Total Audience</p>
                    <p className="text-3xl font-black text-slate-900">{report.total_invites}</p>
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/30">
                    <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-1">Total Returns</p>
                    <p className="text-3xl font-black text-emerald-600">{report.completed_invites}</p>
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/30 md:col-span-2">
                    <div className="flex justify-between items-end mb-2">
                        <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest leading-none">Participation Rate</p>
            ...
            <div className="space-y-6">
                <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Detailed Findings</h2>
                <div className="grid grid-cols-1 gap-6">
                    {report.questions.map((q: any) => (
                        <div key={q.question_id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-100/20 space-y-8 group transition-all duration-500 hover:border-indigo-600">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center px-2">
                                    <div>
                                        <div className="flex justify-between items-end mb-4 pr-1">
                                            <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest">Score Distribution</p>
                                            <p className="text-base font-black text-slate-900 font-mono italic">AVG. {q.average_score?.toFixed(1) || '0.0'}</p>
                                        </div>
                                        <div className="space-y-4">
                                            {['5', '4', '3', '2', '1'].map(val => (
                                    ...
                                    <div className="flex items-center justify-center relative py-10">
                                        <div className="text-center space-y-2 relative z-10">
                                            ...
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-full inline-block">Organizational Pulse</p>
                                        </div>
                            ...
                            {q.question_type === 'TEXT' && (
                                <div className="space-y-4 max-h-[400px] overflow-y-auto no-scrollbar pr-2 pt-2">
                                    {q.text_responses.length > 0 ? q.text_responses.map((resp: string, idx: number) => (
                                        <div key={idx} className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 text-sm font-medium text-slate-600 italic leading-relaxed hover:bg-white hover:shadow-lg hover:shadow-slate-100 transition-all border-l-4 border-l-indigo-600/30">
                                            "{resp}"
                                        </div>
                                    )) : (
                                        <p className="text-slate-300 font-black uppercase tracking-widest text-[9px] text-center py-10 italic">No textual entries were submitted for this item</p>
                                    )}
                                </div>
                            )}
                            ...
                            {q.question_type === 'MCQ' && (
                                <div className="space-y-4">
                                    {Object.entries(q.distribution).map(([opt, count]: [any, any]) => (
