        ...
        <div className="min-h-screen bg-slate-50/50">
            ...
            <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 py-4 px-8 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    ...
                    <button 
                        onClick={handleSave}
                        disabled={submitting || !formData.survey_type_id || !formData.title || formData.questions.length === 0}
            ...
            <main className="max-w-4xl mx-auto p-8 lg:p-12 space-y-12 animate-in fade-in duration-700">
                ...
                <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
                    <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Framework Configuration</h3>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Survey Category</label>
                        ...
                        <div className="md:col-span-2 space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Executive Instructions</label>
                            <textarea 
                ...
                <section className="space-y-6">
                    <div className="flex justify-between items-center px-1">
                        <div className="flex items-center gap-3">
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Question Stack</h3>
                        ...
                        <div className="flex gap-3">
                            <button 
                                ...
                                className="bg-slate-900 text-white text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-indigo-600 px-5 py-2.5 rounded-xl transition-all disabled:opacity-30"
                            >
                            ...
                            <button 
                                type="button" 
                                onClick={addQuestion}
                                className="bg-white border border-slate-200 text-slate-900 text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-slate-50 px-5 py-2.5 rounded-xl transition-all"
                            >
                    ...
                    <div className="space-y-6">
                        {formData.questions.map((q, i) => (
                            <div key={i} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-8 transition-all relative group animate-in slide-in-from-bottom-2 duration-300">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {(["RATING", "TEXT", "MCQ"] as const).map(type => (
                                                <button
                                                    ...
                                                    className={`px-6 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all border ${q.type === type ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
                                                >
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {JSON.parse(q.options || "[]").map((opt: string, optIdx: number) => (
                                                <div key={optIdx} className="flex gap-2 items-center group/opt">
                                                    <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-[10px] font-bold text-slate-300 group-hover/opt:bg-indigo-50 transition-colors shrink-0">{String.fromCharCode(65 + optIdx)}</div>
                                                    <input 
                                                        className="flex-1 px-4 py-2 bg-slate-50 border border-slate-100 rounded-lg text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20"
                                                        value={opt}
                                            ...
                                            <button 
                                                type="button" 
                                                onClick={() => addOption(i)}
                                                className="px-6 py-2.5 border border-dashed border-slate-200 rounded-lg text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:border-indigo-200 hover:text-indigo-600 transition-all flex items-center justify-center gap-2 bg-slate-50/20"
                                            >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Describe Your Industry</label>
                                <input 
                                    ...
                                    autoFocus
                                />
                                <p className="text-[10px] text-slate-400 font-medium px-1 italic">Generated questions will reflect industry nuances.</p>
                            </div>
                            <button 
                                onClick={generateWithAi}
                                disabled={generatingAi || !industryNature}
                                className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-indigo-600 transition-all disabled:opacity-30"
                            >
                                {generatingAi ? (
                                    <>
