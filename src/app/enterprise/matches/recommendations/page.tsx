...
/**
 * Recommendations — for each open job, the people you already have who fit it.
 *
 * NOT Manatal's "Matches": theirs is the list of pairings that already exist, which now lives
 * at /enterprise/matches. This answers the other question — who SHOULD be on this job — and it
 * is worth having for a real reason: most companies have already met the person they are about
 * to spend three weeks sourcing. Someone applied for a different role last quarter, was strong,
 * and lost. Nothing ever surfaces them again.
 *
...
export default function RecommendationsPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const [m, s] = await Promise.all([
                fetch(`${BACKEND_URL}/api/v1/enterprise/matches/recommendations?min_score=${minScore}`, { headers }),
                fetch(`${BACKEND_URL}/api/v1/enterprise/matches/recommendations/summary`, { headers }),
            ]);
            if (!m.ok) {
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            <div className="px-6">
                <PageHeader
                    title={tr("matches.recommendationsTitle")}
                    subtitle={tr("matches.recommendationsSubtitle")}
                    icon="auto_awesome"
                    actions={
                        <div className="flex items-center gap-2">
                            <Link href="/enterprise/matches">
                                <Button size="sm" variant="secondary" icon="format-list-bulleted">{tr("matches.allMatches")}</Button>
                            </Link>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 min-h-0">
                ...
                {loading ? (
                    ...
                    <div className="bg-white border border-[#E0E0E0] rounded-[4px]">
                        <EmptyState icon="auto_awesome" title={tr("matches.emptyTitle")} description={tr("matches.emptyDesc")} />
                    </div>
                ) : (
