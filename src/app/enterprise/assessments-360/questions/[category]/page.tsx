        ...
        <div className="min-h-screen bg-slate-50/50">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Dedicated Competency View</p>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-4">
            ...
            <main className="max-w-[1600px] mx-auto p-12 grid grid-cols-12 gap-10">
                ...
                <aside className="col-span-12 lg:col-span-4 space-y-8">
                    <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-0.5">Define new competency rules</p>
                                </div>
                            </div>
                            
                            <form onSubmit={handleAdd} className="space-y-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Evaluation Metric</label>
                                    <textarea 
                                        className="w-full px-8 py-6 bg-slate-50 border-2 border-transparent rounded-[2rem] focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 outline-none text-sm font-bold text-slate-700 placeholder:text-slate-300 transition-all min-h-[160px] leading-relaxed"
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Metric Architecture</label>
                                    <div className="relative">
                                        <select 
                                            className="w-full px-8 py-4 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-indigo-500 focus:bg-white outline-none text-sm font-black text-slate-700 appearance-none cursor-pointer transition-all"
                                ...
                                <button 
                                    type="submit"
                                    className="w-full h-16 bg-slate-900 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-[0.25em] shadow-xl hover:bg-indigo-600 hover:shadow-indigo-100 transition-all hover:-translate-y-1 active:translate-y-0"
                                >
                                    Inject Competency
                                </button>
                            </form>
                    ...
                    <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/40 overflow-hidden">
                        <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                            <div>
                                ...
                                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] mt-1">Repository for {category.replace(/_/g, ' ')}</p>
                            </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-white">
                                        <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] border-b border-slate-50">Content Archetype</th>
                                        <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] border-b border-slate-50">Metric</th>
                                        <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] border-b border-slate-50 text-right">Actions</th>
                                    </tr>
                                </thead>
                                    ...
                                    {questions.length === 0 ? (
                                        <tr>
                                            <td colSpan={3} className="px-10 py-32 text-center">
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <p className="text-slate-300 text-[10px] font-black uppercase mt-2">Add manual entries or use the AI Generator on the main page.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                        questions.map((q) => (
                                            <tr key={q.id} className="group hover:bg-slate-50/50 transition-all">
                                                ...
                                                <td className="px-10 py-10">
                                                    <span className="text-[10px] font-black uppercase px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl border border-indigo-100 tracking-widest">
                                                        {q.type}
                                                    </span>
                                                </td>
                                                <td className="px-10 py-10 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button className="p-3 text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all shadow-md active:scale-95">
                                                            <span className="material-symbols-rounded text-xl">edit</span>
                                                        </button>
                                                        <button className="p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-2xl transition-all shadow-md active:scale-95">
