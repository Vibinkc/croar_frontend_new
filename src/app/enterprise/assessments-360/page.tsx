        ...
        <div className="p-4 sm:p-5 max-w-7xl mx-auto space-y-6 pt-2 animate-in fade-in duration-700">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-xl border border-slate-100 p-2 shadow-lg shadow-slate-200/20">
                <div className="flex items-center gap-3 px-2">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-[10px] font-medium  ">Enterprise Talent Review Hub</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-lg shadow-slate-100/30 group hover:border-[#7C3AED] transition-all duration-500">
                    ...
                    <p className="text-slate-400 text-[9px] font-black   leading-none mb-1.5">Active Cycles</p>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-lg shadow-slate-100/30 group hover:border-orange-500 transition-all duration-500">
                    ...
                    <p className="text-slate-400 text-[9px] font-black   leading-none mb-1.5">Pending Action</p>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-lg shadow-slate-100/30 group hover:border-emerald-500 transition-all duration-500">
                    ...
                    <p className="text-slate-400 text-[9px] font-black   leading-none mb-1.5">Completed</p>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-lg shadow-slate-100/30 group hover:border-blue-500 transition-all duration-500">
                    ...
                    <p className="text-slate-400 text-[9px] font-black   leading-none mb-1.5">Participants</p>
                ...
                <div className="bg-white rounded-xl border border-slate-100 shadow-xl shadow-slate-200/20 overflow-hidden">
                    <div className="p-4 border-b border-slate-50 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="font-black text-slate-900 text-[11px]  tracking-tight">Manage Active Cycles</h2>
                        </div>
                        <Link href="/enterprise/assessments-360/cycles" className="text-[#7C3AED] text-[9px] font-black   hover:underline">View All Cycles</Link>
                    </div>
                    <div className="p-4 space-y-2">
                        {recentCycles.length > 0 ? recentCycles.map((cycle) => (
                            <div key={cycle.id} className="group p-5 bg-slate-50 rounded-xl hover:bg-slate-900 transition-all duration-500 flex flex-col md:flex-row items-center justify-between gap-4 cursor-pointer" onClick={() => router.push(`/enterprise/assessments-360/cycles/${cycle.id}`)}>
                                <div>
                                    <h3 className="text-lg font-black text-slate-900 group-hover:text-white leading-tight mb-1">{cycle.name}</h3>
                                    <p className="text-[10px] font-bold text-slate-400 group-hover:text-slate-500  ">{new Date(cycle.start_date).toLocaleDateString()} - {new Date(cycle.end_date).toLocaleDateString()}</p>
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="w-8 h-8 bg-white rounded-xl flex items-center justify-center text-slate-400 group-hover:text-[#7C3AED] shadow-sm transition-all transition-transform group-hover:scale-110">
                                        <span className="material-symbols-rounded text-base">trending_up</span>
                                    </div>
                                </div>
                            </div>
                        )) : (
                            <p className="text-slate-400 text-xs font-medium  py-10 text-center border-2 border-dashed border-slate-50 rounded-xl">
