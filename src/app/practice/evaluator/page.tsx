    ...
    if (loading) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-[10px] font-black tracking-[0.3em] text-slate-400 ">Synchronizing_Evaluator_Modules</span>
            </div>
        </div>
    );
            ...
            <section className="relative overflow-hidden rounded-[1.5rem] bg-fuchsia-600 p-6 text-white shadow-lg">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-3">
                        <h2 className="text-2xl font-black  tracking-tight">Neural Evaluator</h2>
                    ...
                    <div className="flex items-center gap-4 bg-white/10 p-4 rounded-xl backdrop-blur-md border border-white/20 min-w-[140px]">
                        ...
                        <div>
                            <p className="text-[9px] font-black text-slate-200   mb-1">Core_Status</p>
                            <div className="flex items-baseline gap-1">
                                <span className="text-2xl font-black">{tasks.length}</span>
                                <span className="text-[9px] font-bold text-slate-200 ">Modules</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div key={task.id} className={`group relative bg-fuchsia-50/50 border border-fuchsia-100 dark:border-slate-800 rounded-2xl p-4 transition-all duration-300 shadow-md hover:shadow-lg ${colors.border} flex flex-col h-full overflow-hidden`}>
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex flex-col items-end gap-1.5">
                                    {task.user_attempt ? (
                                        <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-800 text-[7px] font-black   border border-slate-200">COMPLETED</span>
                                    ) : (
                                        <span className="px-2 py-0.5 rounded-full bg-slate-50 text-slate-400 text-[7px] font-black   border border-slate-100">NOT STARTED</span>
                                    )}
                                    <span className={`px-2 py-0.5 rounded text-[8px] font-black   border ${getDifficultyColor(task.difficulty)} shadow-sm`}>
                                        {task.difficulty}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 flex-grow">
                                <h3 className={`text-base font-black text-slate-900 dark:text-slate-100 tracking-tight leading-tight  group-hover:${colors.iconText} transition-colors line-clamp-2`}>
                                    {task.topic}
                                </h3>
                ...
                {tasks.length === 0 && (
                    <div className="relative border-2 border-dashed border-slate-100 rounded-2xl p-4 flex flex-col items-center justify-center text-center space-y-2 group hover:border-slate-200 transition-colors duration-300 col-span-full h-48">
                        ...
                        <div className="space-y-0.5">
                            <h4 className="text-[10px] font-black text-slate-300   group-hover:text-slate-400 transition-colors">System Idle</h4>
                            <p className="text-[8px] text-slate-300 font-black  ">No Active Modules</p>
                        </div>
                    </div>
                )}
            </div>
