        ...
        <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center">
            <div className="w-full max-w-6xl space-y-10">
                ...
                <div className="text-center space-y-4">
                    <span className="px-3 py-1 bg-slate-900 text-white rounded-full text-[10px] font-black  ">Post-GD Analysis</span>
                    <h1 className="text-5xl font-black text-slate-900  tracking-tighter ">AI Assessment Report</h1>
                ...
                {isLoading ? (
                    <div className="h-[50vh] flex flex-col items-center justify-center space-y-6">
                        ...
                        <p className="font-black text-slate-400   animate-pulse">AI is preparing your report...</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {/* Leaderboard */}
                        <div className="lg:col-span-1 space-y-6">
                            <h3 className="text-xs font-black text-slate-500   px-2">Final Ranking</h3>
                                ...
                                {results.map((res, i) => (
                                    <div key={i} className={`p-6 flex items-center justify-between border-b border-slate-50 last:border-0 ${res.user_id === currentUser?.id ? 'bg-slate-50' : ''}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <p className="font-black text-slate-800  text-sm">{res.user_name}</p>
                                        </div>
                                        {res.user_id === currentUser?.id && <span className="px-2 py-0.5 bg-slate-900 text-white rounded text-[8px] font-black  ">Current User</span>}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <h3 className="text-xs font-black text-slate-500   px-2">Personalized Feedback</h3>
                                    ...
                                    <div key={i} className="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200 border border-slate-100">
                                        <div className="flex justify-between items-start mb-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h4 className="text-xl font-black text-slate-900  tracking-tight">{res.user_name}</h4>
                                                    <p className="text-[10px] font-bold text-slate-500  ">Rank #{res.ranking}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-6">
                                            <div className="space-y-4">
                                                <h5 className="text-[10px] font-black text-slate-900  ">Top Strengths</h5>
                                                <div className="flex flex-wrap gap-2">
                                                    {res.feedback.strengths.map((s, j) => (
                                                        <span key={j} className="px-3 py-1.5 bg-slate-50 text-slate-700 rounded-xl text-xs font-bold border border-slate-200 ">{s}</span>
                                                    ))}
                                                </div>
                                            </div>
                                            <div className="space-y-4">
                                                <h5 className="text-[10px] font-black text-slate-400  ">Areas for Growth</h5>
                                                <div className="flex flex-wrap gap-2">
                                                    {res.feedback.weaknesses.map((w, j) => (
                                                        <span key={j} className="px-3 py-1.5 bg-white text-slate-500 rounded-xl text-xs font-bold border border-slate-100 ">{w}</span>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
