    ...
    if (isLoading) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-[10px] font-black tracking-[0.3em] text-slate-400 uppercase">Loading_Modules</span>
            </div>
        </div>
    );
            ...
            <section className="relative overflow-hidden rounded-[1.5rem] bg-indigo-600 p-6 text-white shadow-lg">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-3">
                        <h2 className="text-2xl font-black uppercase tracking-tight">Async Video Hub</h2>
                    ...
                    <div className="flex items-center gap-4 bg-white/10 p-4 rounded-xl backdrop-blur-md border border-white/20 min-w-[140px]">
                        ...
                        <div>
                            <p className="text-[9px] font-black text-slate-200 uppercase tracking-widest mb-1">Active_Challenges</p>
                            <div className="flex items-baseline gap-1">
                                <span className="text-2xl font-black">{interviews.length}</span>
                                <span className="text-[9px] font-bold text-slate-200 uppercase">Available</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {interviews.map((interview) => (
                    <div key={interview.id} className="group relative bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 transition-all duration-300 shadow-md hover:shadow-lg hover:border-indigo-200 flex flex-col h-full overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex flex-col items-end gap-1">
                                {interview.user_attempt ? (
                                    <span className="px-2 py-0.5 rounded-full bg-slate-100 text-[7px] font-black tracking-widest text-slate-500 uppercase border border-slate-200 animate-pulse">
                                        COMPLETED // {interview.user_attempt.overall_score}%
                                    </span>
                                ) : (
                                    <span className="px-2 py-0.5 rounded-full bg-slate-50 text-[7px] font-black tracking-widest text-slate-400 uppercase border border-slate-100">
                                        PENDING_SEQUENCE
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="space-y-3 flex-grow">
                            <div>
                                <h3 className="text-base font-black tracking-tight leading-tight uppercase transition-colors text-slate-900 group-hover:text-slate-600">
                                    {interview.title}
                                </h3>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-50">
                            {interview.user_attempt ? (
                                <Link href={`/practice/automated-video-interviews/${interview.id}/result`}>
                                    <button className="w-full bg-slate-100 hover:bg-slate-900 text-slate-900 hover:text-white font-black text-[9px] tracking-[0.2em] uppercase py-2.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 active:scale-95 shadow-sm shadow-slate-100">
                                        VIEW_RESULTS
            ...
            {interviews.length === 0 && (
                <div className="bg-white rounded-[2.5rem] p-20 border border-slate-100 flex flex-col items-center justify-center text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">No Active Challenges</h2>
