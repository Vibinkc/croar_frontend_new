...
function CodingList({ divisionId, departmentId }: { divisionId: number | null, departmentId: number | null }) {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 animate-in fade-in duration-700">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mt-8 animate-pulse">Synchronizing_Vaults</p>
        </div>
    );
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <span className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[9px] font-black uppercase tracking-widest rounded-lg border border-indigo-100">Module Sector</span>
                                <span className="text-slate-300 text-xs">/</span>
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{selectedTopic}</span>
                            </div>
                            <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase leading-none">{selectedTopic}</h1>
                        </div>
                    </div>
                    <div className="px-6 py-3 bg-slate-50 rounded-2xl border border-slate-100">
                        <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-3">
                        ...
                        <div key={q.id} className="group bg-white p-4 rounded-xl border border-slate-100 hover:border-indigo-100 hover:shadow-sm transition-all duration-300 flex items-center justify-between relative overflow-hidden">
                            ...
                            <div className="flex-1 pr-12 relative z-10">
                                <div className="flex items-center gap-4 mb-4">
                                    <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all duration-500 ${q.difficulty === 'HARD' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                                        q.difficulty === 'MEDIUM' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                                            'bg-emerald-50 text-emerald-600 border-emerald-100'
                                        }`}>
                                        {q.difficulty}
                                    </span>
                                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">Protocol ID: #{q.id}</span>
                                </div>
                                <h3 className="text-sm font-bold text-slate-600 leading-relaxed group-hover:text-slate-900 transition-colors uppercase tracking-tight">
                                    {q.content.question}
                                </h3>
                            </div>
    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-4 pb-12 px-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                <div>
                    <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase leading-none mb-2">Algorithmic Vaults</h1>
            ...
            {Object.keys(groupedQuestions).length === 0 ? (
                <div className="flex flex-col items-center justify-center p-24 bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-100 text-center animate-in fade-in zoom-in duration-1000">
                    ...
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-2 text-indigo-900">Vault_Depleted</h3>
                    <p className="text-[10px] text-slate-400 font-bold max-w-xs uppercase tracking-[0.2em] leading-loose">No neural protocols identified for the current sector. Initialize the gateway to begin ingestion.</p>
                </div>
            ) : (
                        ...
                        >
                            <div className="flex justify-between items-start mb-8">
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    <span className="px-4 py-1.5 rounded-xl bg-slate-900 text-[10px] font-black text-white uppercase tracking-[0.1em] border border-slate-800 shadow-lg shadow-slate-100">
                                        {topicQuestions.length} Protocols
                                    </span>
                                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.2em]">Coding_Bank_v2</span>
                                </div>
                            </div>

                            <div className="mb-8">
                                <div className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em] mb-2 px-1">Active Module</div>
                                <h3 className="text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight leading-none mb-3 truncate">{topic}</h3>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {topicQuestions.slice(0, 3).map((q, idx) => (
                                        <div key={q.id || idx} className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-100 rounded-2xl text-[9px] text-slate-500 font-bold transition-all hover:bg-white hover:border-indigo-100 group/item">
                                            ...
                                            <span className="truncate max-w-[100px] uppercase">P_ID#{q.id}</span>
                                        </div>
                                    ))}
                                    {topicQuestions.length > 3 && (
                                        <div className="text-[9px] font-black text-slate-400 py-2 px-4 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 uppercase tracking-widest">
                            ...
                            <div className="flex items-center gap-4 pt-8 border-t border-slate-50 mt-auto relative z-10">
                                <button
                                    ...
                                    className="flex-1 py-4 rounded-2xl bg-slate-50 text-slate-500 hover:bg-indigo-900 hover:text-white text-[10px] font-black uppercase tracking-[0.2em] text-center transition-all shadow-sm active:scale-95 group/btn"
                                >
