...
function VideoInterviewList({ divisionId, departmentId }: { divisionId: number | null, departmentId: number | null }) {
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center p-20 animate-in fade-in duration-500">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Loading_Interview_Vault</p>
        </div>
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase leading-none mb-1.5">Simulation Repository</h1>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <table className="min-w-full divide-y divide-slate-50">
                    <thead className="bg-slate-50/50">
                        <tr>
                            <th className="px-6 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Protocol / Title</th>
                            <th className="px-6 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Initialization_Date</th>
                            <th className="px-6 py-4 text-right text-[9px] font-black text-slate-400 uppercase tracking-widest flex justify-end pr-6">Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colSpan={3} className="px-6 py-10 text-center">
                                    <div className="flex flex-col items-center justify-center text-slate-300">
                                        ...
                                        <p className="text-[9px] font-black uppercase tracking-[0.2em]">No_Video_Protocols_Found</p>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                                ...
                                <tr key={interview.id} className="hover:bg-slate-50/80 transition-all group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="text-xs font-black text-slate-900 uppercase tracking-tight">{interview.title}</div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
