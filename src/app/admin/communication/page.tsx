...
function CommunicationList({ divisionId, departmentId }: { divisionId: number | null, departmentId: number | null }) {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 animate-in fade-in duration-700">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mt-8 animate-pulse">Synchronizing_Realities</p>
        </div>
    );
    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-12 pb-32 px-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase leading-none mb-2">Communication Vaults</h1>
            ...
            {scenarios.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-100 text-center animate-in fade-in zoom-in duration-1000">
                    ...
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-1 text-indigo-900">Vault_Empty</h3>
                    <p className="text-[9px] text-slate-400 font-bold max-w-xs uppercase tracking-[0.2em] leading-loose">No communication protocols identified for the current sector. Initialize the gateway to begin scripting.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-6 relative z-10">
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all duration-500 ${s.difficulty === 'HARD' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                                        s.difficulty === 'MEDIUM' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                                            'bg-emerald-50 text-emerald-600 border-emerald-100'
                                        }`}>
                                        {s.difficulty}
                                    </span>
                                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.2em]">Scenario_v1.2</span>
                                </div>
                            </div>

                            <div className="mb-4 relative z-10 flex-1 flex flex-col">
                                <div className="text-[8px] font-black text-slate-300 uppercase tracking-[0.3em] mb-1 px-1">Simulation Protocol</div>
                                <h3 className="text-base font-black text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight leading-tight mb-3 line-clamp-2">{s.title}</h3>

                                <div className="bg-slate-50/80 p-5 rounded-2xl border border-dashed border-slate-100 group-hover:bg-white group-hover:border-indigo-100 transition-all duration-500 italic flex-1">
                                    <p className="text-[10px] font-bold text-slate-400 line-clamp-4 leading-relaxed uppercase tracking-wide">
                                        "{s.prompt}"
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 pt-8 border-t border-slate-50 relative z-10 mt-auto">
                                <Link
                                    ...
                                    className="flex-1 bg-white hover:bg-slate-900 border border-slate-100 hover:border-slate-900 text-slate-400 hover:text-white py-2.5 rounded-xl text-[8px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all shadow-sm active:scale-95 group/btn"
                                >
                    ...
                    >
                        ...
                        <div className="text-center">
                            <h4 className="text-lg font-black text-slate-900 uppercase tracking-tight mb-2 leading-none">Initialize New Simulation</h4>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Construct more soft-skill realities</p>
                        </div>
                    </Link>
                </div>
            )}
