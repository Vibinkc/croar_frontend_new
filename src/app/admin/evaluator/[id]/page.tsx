...
export default function EvaluatorResults() {
    ...
    return (
        <EvaluatorScoreList questionId={questionId} />
    );
}

function EvaluatorScoreList({ questionId }: { questionId: string }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await apiClient.get(`/api/v1/content/questions/${questionId}/attempts`);
                if (res.ok) {
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {attempts.map((att) => (
                                <tr key={att.id} className="group hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-6">
                                        <div className={`inline-flex items-center px-3 py-1 rounded-full border ${att.score >= 80 ? "bg-emerald-50 border-emerald-100 text-emerald-600" :
                                            att.score >= 50 ? "bg-amber-50 border-amber-100 text-amber-600" :
                                                "bg-rose-50 border-rose-100 text-rose-600"
                                            }`}>
                                            <span className="text-[10px] font-black">{att.score ? att.score.toFixed(0) : 0}%</span>
                                        </div>
                                    </td>
                                    <td className="p-6">
                                        <div className="flex gap-2 text-[9px] font-black text-slate-500">
                                            <span title="Grammar" className="bg-slate-100 px-1.5 py-0.5 rounded">G: {att.grammar_score}</span>
                                            <span title="Relevance" className="bg-slate-100 px-1.5 py-0.5 rounded">R: {att.relevance_score}</span>
                                        </div>
                                    </td>
                                    <td className="p-6">
                                    ...
                                    <td className="p-6">
                                        <p className="text-[11px] font-medium text-slate-400 line-clamp-2 max-w-xs leading-relaxed">
                                            {att.feedback}
                                        </p>
                                    </td>
                                </tr>
