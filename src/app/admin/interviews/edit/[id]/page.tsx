...
export default function EditInterviewPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFormData({
                    ...
                    strictness: data.settings.strictness,
                    avatar: data.avatar_config.avatar,
                    voice: data.avatar_config.voice
                });
                                ...
                                <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 hover:border-slate-300 transition-all group">
                                    ...
                                    <div className="space-y-3">
                                        {m.questions.map((q, qIdx) => (
                                            <div key={qIdx} className="flex gap-3 items-start bg-white/50 p-2 rounded-xl border border-dashed border-slate-200 group-hover:border-slate-300 transition-all">
                                                <input
                                                    type="text"
                                                    value={q}
                                                    onChange={(e) => handleEditQuestion(idx, qIdx, e.target.value)}
                                                    className="flex-1 bg-transparent border-none focus:ring-0 text-sm text-slate-600 outline-none"
                                                />
                                                <button
    ...
    return (
        <div className="min-h-[80vh] flex items-center justify-center p-4">
            <AIGenerationOverlay isOpen={isLoading && step === 1} title="Re-Engineering Interview Strategy" />
            <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                {/* Progress Bar */}
                {step < 6 && (
