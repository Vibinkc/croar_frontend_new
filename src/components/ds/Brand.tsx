...
export function CroarMark({ size = 36, className, glow = true }: { size?: number; className?: string; glow?: boolean }) {
    ...
    return (
        <div
            className={cn("flex items-center justify-center rounded-[4px] shrink-0", className)}
            style={{ width: size, height: size, background: BRAND_GRADIENT, boxShadow: glow ? "0 6px 18px rgba(25,118,210,0.45)" : undefined }}
        >
