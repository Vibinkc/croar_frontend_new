    ...
    return (
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="text-xs font-black text-slate-400  ">{t("superAdmin.questionPreview")}</h3>
                <span className="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-black rounded-lg ">{t("superAdmin.typeSpotOn")}</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Target Pattern */}
                <div className="flex flex-col items-center justify-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <span className="text-[10px] font-black text-slate-400   mb-4">{t("superAdmin.targetPattern")}</span>
                ...
                <div className="space-y-4">
                    <span className="text-[10px] font-black text-slate-400  ">{t("superAdmin.options")}</span>
                        ...
                        {options?.map((opt, idx) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {idx === correctIndex && (
                                        <span className="absolute -bottom-2 px-2 py-0.5 bg-gray-500 text-white text-[8px] font-black rounded ">{t("superAdmin.correct")}</span>
                                    )}
                                </div>
                            );
                        })}
